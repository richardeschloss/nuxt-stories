...
export default defineNuxtPlugin(async (nuxtApp) => {
  ...
  const { staticHost, storiesAnchor, lang: langDefault } = $config.public.nuxtStories
  const anchorRegex = new RegExp(`/${storiesAnchor}/*$`)

  addRouteMiddleware((to) => {
    if (anchorRegex.test(to.path)) {
      // TBD: Re-enable
      // return navigateTo(`/${storiesAnchor}/${langDefault}/`)
    }
  })
