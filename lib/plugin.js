...
export const register = Object.freeze({
  ...
  async plugins() { // TBD: can probably remove...
    if (process.client) {
      // debug('[plugin] register bootstrap plugins')
    ...
    router.beforeEach((to, _, next) => {
      ...
      if (to.path.startsWith(`/${storiesAnchor}`) && !to.params.lang) {
        ...
        if (pathParts.length === 1) {
          ...
          router.push(`/${storiesAnchor}/${lang}`) // TBD: append "/"?
        } 
...
export default async function({ app, $config, store }, inject) {
