...
import NuxtStoriesHeader from './components/Header.js'
import NuxtStoriesSideNav from './components/SideNav.js'
import NuxtStoriesMain from './components/Main.js'
import { defineNuxtPlugin, useState } from '#app'
import layouts from '#build/layouts'

layouts.stories = StoriesLayout

const debug = Debug('nuxt-stories')

const components = { NuxtStoriesHeader, NuxtStoriesSideNav, NuxtStoriesMain, Json, TestRunner, TestCoverage, Readme }
...
export default defineNuxtPlugin(async (nuxtApp) => {
  ...
  nuxtApp.vueApp.config.errorHandler = (err, vm, info) => {
    ...
    if (info === 'render') {
      ...
      nuxtApp.vueApp.$nuxt.$nuxtStories().value.compileErr = err.message
      // throw new Error(err)
    }
  }
  ...
  nuxtApp.vueApp.config.warnHandler = (err, vm, info) => {
    ...
    if (typeof err === 'string' && err.includes('Vue received a Component which was made a reactive object')) {
      return
    }
    ...
    if (info && info.includes('VueJsonPretty')) {
      ...
      return
    } else if (err.includes('Template compilation error') || err.includes('not defined on instance')) {
