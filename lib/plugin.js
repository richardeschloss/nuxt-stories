...
export default defineNuxtPlugin(async (nuxtApp) => {
  ...
  if (process.client) { // TBD: styleEditor mounted hook
    console.log('load style...')
    nuxtStories().value.activeStyle = localStorage.getItem('activeStyle')
    // nuxtStories().value.stylingComp = localStorage.getItem('stylingComp')
    nuxtStories().value.styles = JSON.parse(localStorage.getItem('styles') || '{}')
    console.log('load style...', nuxtStories().value.stylingComp)
  }
