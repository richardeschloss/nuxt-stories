...
export const register = Object.freeze({
  ...
  async ioServer (nuxt, ioOpts = {}, server) {
    ...
    if (nuxt?.options?.devServer?.port) {
      const { host, port, https, url } = nuxt.options.devServer
      const proto = https ? 'https' : 'http'
      useOpts.cors = {
        origin: url,
        methods: ['GET', 'POST']
      }
      Object.assign(useOpts, { host: host === '' ? 'localhost' : host, port: parseInt(port) + 100, proto })
    }
  ...
  ioClient (nuxt, ioOpts = {}) {
    ...
    if (nuxt?.options?.devServer?.port) {
      ...
      Object.assign(useOpts, { host, port: parseInt(port) + 100 })
    } else {
    ...
    if (useOpts.host !== undefined) {
      const url = useOpts.url || `${useOpts.proto}://${useOpts.host === '' ? 'localhost' : useOpts.host}:${useOpts.port}`
      const storiesSocket = {
        name: 'nuxtStories',
        url
      }
