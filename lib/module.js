...
export const register = Object.freeze({
  ...
  async db ({ srcDir, storiesDir }) {
    db = await DB({ 
      srcDir,
      filename: pResolve(srcDir, storiesDir, 'stories.db')
    })
  ...
  middlewares (ctx, storiesDir, staticHost = {}) {
    ...
    const middlewares = [
      // Example: fetch /markdown/stories.json 
...
export default async function (moduleOptions) {
  ...
  this.nuxt.hook('modules:done', async (moduleContainer) => {
    await register.db({ srcDir, storiesDir })
    const storyRoutes = register.routes({ storiesDir })
    pOptions.stories = register.stories(lang)
