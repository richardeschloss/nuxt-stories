...
export const register = Object.freeze({
  ...
  middlewares (storiesDir, staticHost = {}) {
    ...
    const middlewares = [
      ...
      {
        path: '/nuxtStories/svg', // If file not in in public/ then we pull from here...
        handler: serveStatic(pResolve(__dirname, 'assets/svg'), {
          maxAge: '1d'
        })
      },
      {
        path: '/nuxtStories/README.md', // If file not in in public/ then we pull from here...
...
export default defineNuxtModule({
  ...
  async setup (moduleOptions, nuxt) {
    ...
    includeDeps(nuxt, [
      'ieee754', 'base64-js', 'highlight.js', 'gray-matter', 'vue-json-pretty'
    ])
