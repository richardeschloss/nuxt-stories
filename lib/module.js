...
import { defineNuxtModule, addPlugin, addDevServerHandler, extendPages, installModule } from '@nuxt/kit'
...
export const register = Object.freeze({
  ...
  async ioServer (nuxt, ioOpts = {}, server) {
    ...
    if (nuxt?.options?.server?.port) {
      const { host, port, https } = nuxt.options.server // TBD: might be deprecated
  ...
  middlewares (storiesDir, staticHost = {}) {
    ...
    const middlewares = [
      ...
      { route: mount, handler: (req, res, next) => serveStatic(`./${storiesDir}`)(req, res, next) },
      {
        route: '/nuxtStories/svg', // If file not in in public/ then we pull from here...
        handler: (req, res, next) => serveStatic(pResolve(__dirname, 'assets/svg'), {
          maxAge: '1d'
        })(req, res, next)
      },
      {
        route: '/nuxtStories/README.md', // If file not in in public/ then we pull from here...
        handler (_, res, next) { // .. and cache it
      ...
      },
      {
        ...
        handler: (req, res, next) => serveStatic(pResolve('./coverage'))(req, res, next)
      }
    ]
