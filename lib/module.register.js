...
export const register = Object.freeze({
  cache (dirs = [
    './components/nuxtStories',
    pResolve(__dirname, './cache')
  ]) {
    dirs.forEach((d) => {
      if (!existsSync(d)) {
        mkdirSync(d)
  ...
  css (ctx, codeStyle) {
    ctx.options.css.push(
      `highlight.js/styles/${codeStyle}.css`,
  ...
  middlewares (ctx, storiesDir, staticHost = {}) {
    ...
    if (_modOptions.fetch) {
      middlewares.push({
        path: '/nsCache', handler: serveStatic(pResolve(__dirname, './cache'))
      })
    }
