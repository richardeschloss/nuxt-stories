...
import { cnt, buildTree, search, parseHref, getLangs } from './db.client.js'
...
export default function DB ({
  ...
  async function fileChanged (evt, file) {
    ...
    if (cbs.fileChanged) {
      const changed = items.findOne({ href })
      const { lang } = parseHref(href)
      const stories = svc.buildTree(lang)
      // eslint-disable-next-line node/no-callback-literal
      cbs.fileChanged.forEach(cb => cb({ stories, changed }))
    }
  }
