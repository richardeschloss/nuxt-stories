...
export const mutations = {
  ...
  ESM_FETCHED({ esms }, { aliases, url, mods, path }) {
    console.log('ESM_FETCHED info!', { aliases, url, mods, path })
    if (!esms[path]) {
      Vue.set(esms, path, {})
    }
    aliases.forEach((alias, idx) => {
      if (!esms[path][alias]) {
        Vue.set(esms[path], alias, {})
      }
      Vue.set(esms[path], alias, {
        mod: mods[idx].default || mods[idx],
        url
      })
      Vue.prototype[alias] = mods[idx].default || mods[idx]
      // Vue.set(esms[path][alias], 'url', 'prior' + url.slice(0) + 'ayyyyy')
      // Vue.set(esms[path][alias], 'mod', mods[idx])
      // esms[path][alias] = url
      // esms[path][alias] = {
      //   mod: mods[idx].default || mods[idx],
      //   url
      // }
      console.log('vuex esms', esms[path][alias])
    })
  },
...
// TBD 
const buildQueue = (arrIn, cacheObj) => {
  return arrIn.reduce((arr, entry) => {
    let alias, url
    ...
    if (!cacheObj[alias] || cacheObj[alias].url !== url) { // And check urls fetched...
      console.log('url diff?', cacheObj[alias], url)
      arr.push([alias, url, typeof entry])
    }
    return arr
  }, [])
}
 ...
 * 
 */
const importStr = (key, alias, url) => {
  const templates = {
    allAsAlias: `import * as ${alias} from "${url}"\n`,
    aliasCSV: `import { ${alias} } from "${url}"\n`,
    aliasCSValt: `import("${url}").then(r => {
      if (!window.$nsImport) {
        window.$nsImport = {}
      } 
      window.$nsImport["${alias}"] = r
    })`
  }
  return templates[key]
}

const commitStr = (label, alias, info, cbStr) => 
  `window.${cbStr}('${label}', `
  + `{ mods: [${alias}], ...${info} })\n`
//
...
export const actions = {
  ...
  FETCH_ESMS ({ commit }, { items, path, notify }) {
    const cache = window.$nuxt.$store.state 
      .$nuxtStories.esms[path] || {} //Vue.prototype)
    const queue = buildQueue(items, cache) 
    ...
    queue.forEach(async ([alias, url, entryType]) => { 
      ...
      if (entryType === 'string') {
        const aliases = [alias]
        window.$nuxt.$store.commit('$nuxtStories/ESM_FETCHED', { 
          aliases, 
          url, 
          mods: aliases.map((m) => ({})), 
          path 
        })
        module.text = importStr('allAsAlias', alias, url) 
          + commitStr(
              '$nuxtStories/ESM_FETCHED', 
              alias, 
              JSON.stringify({ path, aliases, url })
            )
      } else {
        ...
        module.text = importStr('aliasCSV', alias, url) 
        + commitStr(
            '$nuxtStories/ESM_FETCHED', 
            alias, 
            JSON.stringify({ 
