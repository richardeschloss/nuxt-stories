...
export const actions = {
  ...
  async FETCH_SCRIPTS({ commit }, { scripts }) {
    ...
    function scriptLoaded(alias, retry) {
      return new Promise((resolve) => {
        setTimeout(() => {
          if (window[alias] || retry > 2) {
            Vue.prototype[alias] = window[alias]
            resolve()
          } else {
            return scriptLoaded(alias, ++retry)
          }
        }, 500)
      })
    }
    
    const p2 = scripts.map((item) => { // Works too
      console.log('load script', item)
      const parts = item.split('/')
      const scriptName = parts[parts.length-1].split('.')[0]
      const script = document.createElement('script')
      script.src = item // `/nsCache/${scriptName}.js`
      const alias = aliases[scriptName] || scriptName
      document.getElementsByTagName('head')[0].appendChild(script)
      console.log('alias loaded?', alias)
      return scriptLoaded(alias, 0)
    })
    ...
    // Works:
    await fetchScripts() 
    const p = queue.map((item) => {
      const parts = item.split('/')
      const scriptName = parts[parts.length-1].split('.')[0]
      const script = document.createElement('script')
      script.src = `/nsCache/${scriptName}.js`
      const alias = aliases[scriptName] || scriptName
      document.getElementsByTagName('head')[0].appendChild(script)
      return scriptLoaded(alias, 0)
    })
