...
function buildESMImporter(items, path, commit) {
  const imports = []
  const mods = []
  items.forEach((item) => {
    let importStr
...
export const mutations = {
  ...
  ESMS_FETCHED({ esmsFetched }, { mods, path }) {
    if (!esmsFetched[path]) {
      Vue.set(esmsFetched, path, {})  
    }
    Object.keys(esmsFetched[path]).forEach(dest => {
      delete Vue.prototype[dest]
    })
    Vue.set(esmsFetched, path, {})  
    Object.entries(mods).forEach(([dest, mod]) => {
      Vue.set(esmsFetched[path], dest, false) 
      Vue.prototype[dest] = mod.default || mod
    })

    Object.keys(mods).forEach((dest) => {
      Vue.set(esmsFetched[path], dest, true) 
    })
  },
...
export const actions = {
  ...
  FETCH_ESMS (_, { items, path }) {
    //TBD: can I do module.onload here to make code simpler?. will the imports be available?
    const id = `nuxt-stories-esm-${path.replace(/\//g, '-')}`
    const commit = '$nuxtStories/ESMS_FETCHED'
    let module = document.getElementById(id)
    const t = buildESMImporter(items, path, commit)
    if (!module) {
