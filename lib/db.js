...
import * as LokiDB from '@lokidb/loki' // TBD: re-test...
import * as LokiDBSearch from '@lokidb/full-text-search'
import * as LokiStorage from '@lokidb/fs-storage'
const { default: Loki } = LokiDB
const { default: FullTextSearch } = LokiDBSearch
const { FSStorage } = LokiStorage
FullTextSearch.register()
FSStorage.register() // server-side
...
export default async function DB({
  ...
  const svc = {
    ...
    async upsert(file, oldFile, srcDir, storiesDir, lang) {
      // TBD
      items.findAndUpdate({
