...
export default {
  render () {
    const { storiesAnchor, lang: langDflt, editable } = this.$config.nuxtStories
  ...
  mounted () {
    const { editable } = this.$config.nuxtStories
    if (!editable) {
  ...
  methods: {
    setViewMode (mode) {
      ...
      localStorage.setItem('nuxtStoriesViewMode', mode)
    },
    updateOverflow () {
