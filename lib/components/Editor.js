export default {
  props: ['story', 'scrollTo', 'viewerScrollTop'],
  render(h) {
    const children = [
  ...
  methods: {
    ...
    async scrollToHref({ depth, text }) {
      const regex = new RegExp(`#{${depth}}\\s*${text}`)
      const fnd = this.input.match(regex)
      if (fnd) {
