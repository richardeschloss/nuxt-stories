...
function toStyleJson (elm) {
  return elm.getAttribute('style').split(/\s*;\s*/)
    .reduce((out, pair) => {
      if (pair !== '') {
...
export default {
  render () {
    ...
    const fontSelect = h(Dropdown, {
      text: this.formatting.selectedFont ? this.formatting.selectedFont : 'Aa',
      ...
      onItemSelected: (item) => {
        const btn = 'selectedFont'
        this.formatting.selectedFont = item.value
        if (this.formatting[btn]) {
          this.state.activeStyle = this.state.activeStyle.trim() +
            `\r\nfont-family: ${this.formatting.selectedFont};\r\n`
        } else {
          this.state.activeStyle = this.state.activeStyle
            .replace(regex['font-family'], `\r\nfont-family: ${this.formatting.selectedFont};\r\n`)
        }
      }
    })
      ...
      Object.entries(this.state.styles).forEach(([selector, style]) => {
        ...
        if (elm) {
          if (selector === this.state.stylingSelector) {
            ...
            this.appliedStyles = toStyleJson(elm)
          }
          elm.style = style
        }
      })
