...
export default {
  ...
  methods: {
    commitChanges () {
      this.saveStyles(this.state.styles, this.savePath)
    },
    collapseModal () {
      this.modalWidth = '15%'
      this.modalHeight = '10%'
      ...
      Object.entries(this.state.styles).forEach(([selector, style]) => {
        ...
        if (elms && elms.length > 0) {
          if (selector === this.state.stylingSelector) {
            ...
            this.input = this.state.styles[selector]
          }
    ...
    updateToolbars () {
      ...
      Object.keys(this.formatting).forEach((style) => {
    ...
    updateStyles () {
      if (this.state.stylingElms.length > 0) {
        this.updateElmsStyle(this.state.stylingElms, this.input)
        this.updateToolbars()
