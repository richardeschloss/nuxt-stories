...
import { compile as compileVue } from '../utils/vue.esm-bundler.js' // TBD: to make npm build succeed, need to use local version instead of released 'vue/dist/vue.esm-bundler.js'
...
export default {
  ...
  render () {
    ...
    if (this.compiledVue) {
      children.push(h(this.compiledVue))
    }
  ...
  computed: {
    compilationError () {
      return this.$nuxtStories().value?.compilationError
    }
  },
  data () {
    return {
  ...
  async mounted () {
    ...
    //   // TBD: style editor stuff?
    //   // pop-up style editor
    //   // accept changes
    //   c.$el.addEventListener('dblclick', (evt) => {
    //     console.log('double clicked in', c.$options.name, evt.target.id)
    //     // save #[id] { styles } in assets/[componentName].scss
    //     // Best way and place to save? TBD...
    //   })
    // })
  },
  beforeUnmount () {
