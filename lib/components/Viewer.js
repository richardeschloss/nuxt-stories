...
export default {
    ...
    return h('div', {
      ...
      onDblclick: (evt) => {
        ...
        if (elm.id) {
          ...
          this.state.activeStyle = this.state.styles[this.state.stylingSelector]
      ...
      onMouseover: (evt) => {
        const elm = evt.target
        if (elm.title === '') {
          elm.title = 'Ctrl + DblClick to edit style'
        }
      }
    }, children)
  },
  ...
  async mounted () {
    ...
    //   console.log('double clicked in', evt.target)
    //   this.state.styleElm = evt.target
    //   const compName = evt.target.__vueParentComponent.type.name
    //   if (compName) {
    //     this.state.activeStyle = this.state.styles[compName]
    //     this.state.stylingComp = compName
    //   }
