  ...
  methods: {
    async compileVue () {
      try {
        ...
        const { imported } = await LoadDeps({ cfg: this.$config.nuxtStories, frontMatter })
        this.compiledVue = StoryFactory({ cfg: this.$config.nuxtStories, frontMatter, imported, ...res })
      } catch (err) {
