...
// import Markdown from './Markdown.vue' // TBD
...
const preCrud = {
  ...
  methods: {
    // updateRoutes() {
    //   // Expect the next level to be defined
    //   // in the routes...
    //   const storiesDepth = this.level + 2
    //   const fnd = this.$router.getRoutes()
    //     .find(({ path }) => path.includes(`:L${storiesDepth}?`))
    ...
    //       component: Markdown
    //     }
    //     this.$router.addRoute(storiesAnchor, routeCfg)
    //   }
    // },
    async addStory() {
...
const storyChildren = {
  ...
  render(h) {
    ...
    return h('div', {
      ...
      class: {
        show: showChildren,
        // collapsing: !showChildren
      },
      attrs: {
        id: `${story.name}-children`
      },
      on: {
        mouseenter: () => {
          this.$emit('hideCRUD')
...
const storyElm = {
  ...
  render(h) {
    ...
    // TBD: test deeper stories before deleting
    // let collapse
    // const currentHref = decodeURIComponent(this.$route.path)
    // let showChildren = currentHref.startsWith(story.href)
    // this.$nextTick(() => {
    ...
    const storyView = [
      h(viewTag, {
        ...
        attrs: {
          ...
          'data-bs-target': `#${story.name}-children`
        },
        on: {
          click: () => {
...
export default {
  ...
  async mounted() {
    // TBD: Better home? Use data-bs-toggle?
    // const Collapse = (await import('bootstrap/js/src/collapse.js')).default   //import('bootstrap/dist/js/bootstrap.esm.js')
    // _Collapse = Collapse
  }
}