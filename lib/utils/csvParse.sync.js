...
Writable.prototype.setDefaultEncoding = function setDefaultEncoding (encoding) {
  ...
  if (!(['hex', 'utf8', 'utf-8', 'ascii', 'binary', 'base64', 'ucs2', 'ucs-2', 'utf16le', 'utf-16le', 'raw'].includes((encoding + '').toLowerCase()))) { throw new TypeError('Unknown encoding: ' + encoding) }
  ...
  prepend (val) {
    if (isBuffer(val)) {
      ...
      if (length >= this.size) {
        this.resize()
        if (length >= this.size) {
          throw new Error('INVALID_BUFFER_STATE')
        }
      }
      const buf = this.buf
  ...
  __normalizeOptions (opts) {
    ...
    if (options.from === undefined || options.from === null) {
      ...
      if (Number.isInteger(options.from)) {
        ...
        throw new TypeError(`Invalid Option: from must be an integer, got ${JSON.stringify(options.from)}`)
      }
    }
    ...
    if (options.from_line === undefined || options.from_line === null) {
      ...
      if (Number.isInteger(options.from_line)) {
        ...
        throw new TypeError(`Invalid Option: from_line must be an integer, got ${JSON.stringify(opts.from_line)}`)
      }
    }
    ...
    if (options.objname === undefined || options.objname === null || options.objname === false) {
      ...
      throw new TypeError(`Invalid Option: objname must be a string or a buffer, got ${options.objname}`)
    }
    if (options.objname !== undefined) {
      if (typeof options.objname === 'number') {
        if (options.columns !== false) {
          throw new Error('Invalid Option: objname index cannot be combined with columns or be defined as a field')
        }
      } else { // A string or a buffer
        if (options.columns === false) {
          throw new Error('Invalid Option: objname field must be combined with columns or be defined as an index')
        }
      }
    }
    ...
    if (options.to === undefined || options.to === null) {
      ...
      if (Number.isInteger(options.to)) {
        ...
        throw new TypeError(`Invalid Option: to must be an integer, got ${JSON.stringify(opts.to)}`)
      }
    }
    ...
    if (options.to_line === undefined || options.to_line === null) {
      ...
      if (Number.isInteger(options.to_line)) {
        ...
        throw new TypeError(`Invalid Option: to_line must be an integer, got ${JSON.stringify(opts.to_line)}`)
      }
    }
    this.info = {
      bytes: 0,
